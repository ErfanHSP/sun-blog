...
if (!data.success) {
    ...
    if (data.validationError) {
        errorBox[0].innerHTML = "Phone number is not valid."
    } else {
        verifyOtpForm.style.display = "flex"
        requestOtpForm.style.display = "none"
        errorBox[0].innerHTML = `${data.message} <br> Try again after ${data.remainingTime} seconds`|| "Something went wrong."
    }
...
verifyOtpForm.addEventListener("submit", (e) => {
    ...
    if (!data.success) {
        ...
        successBox[0].style.display = 'block'
        errorBox[0].style.display = 'none'
        successBox[0].innerHTML = "Login was successful. Redirecting..."
        return location.replace("/me/panel")
    }
    })
    .catch((err) => {
    console.error("Network error:", err)
